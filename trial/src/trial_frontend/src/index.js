...
document.addEventListener(
  ...
  async (e) => {
    ...
    const question = await trial_backend.get_question();
    ...
    const voteCounts = await trial_backend.get_votes();
    updateLocalVoteCounts(JSON.parse(voteCounts));
    displayResults();
    return false;
  },
  false
...
pollForm.addEventListener(
  ...
  async (e) => {
    ...
    const updatedVoteCounts = await trial_backend.vote(checkedValue);
    console.log("Returning from await...");
    console.log(updatedVoteCounts);
    updateLocalVoteCounts(JSON.parse(updatedVoteCounts));
    displayResults();
    return false;
  },
  false
...
resetButton.addEventListener(
  ...
  async (e) => {
    ...
    await trial_backend.reset_votes();
    const voteCounts = await trial_backend.get_votes();
    updateLocalVoteCounts(JSON.parse(voteCounts));
...
// [["Motoko","0"],["Python","0"],["Rust","0"],["TypeScript","0"]]
  ...
  function updateLocalVoteCounts(arrayOfVoteArrays) {
    for (let voteArray of arrayOfVoteArrays) {
      ...
      let voteCount = parseInt(voteArray[1]);
      pollResults[voteOption] = voteCount;
      console.log(voteOption, voteCount);
    }
  }
